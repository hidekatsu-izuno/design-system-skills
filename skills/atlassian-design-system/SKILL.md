...
description: Use when the task asks for UI design, frontend implementation, dashboards, issue-tracking style interfaces, or static mocks that should follow Atlassian Design System.
---

# Atlassian Design System Skill

Use this skill when the output should look and behave like Atlassian Design System. Read [./DESIGN.md](./DESIGN.md) first and treat it as the source of truth for layout, components, colors, typography, and UI constraints.

## Working Model

- Read `./DESIGN.md` before making any visual or structural decision.
- Extract the system's shell, navigation, lozenge/status, message, and dense work-management patterns before writing code.
- Adapt the requested information architecture into Atlassian patterns instead of defaulting to a generic dashboard shell.
- Prefer documented Atlassian component logic and naming whenever multiple solutions are plausible.
...
- Implement with documented Atlassian-style patterns first, especially for side navigation, lozenges, inline status, and operational layouts.
...
- Do not flatten the result into a generic enterprise dashboard if Atlassian implies a more specific shell or workflow surface.
...
- Keep the result recognizably Atlassian at the shell, component, spacing, and state level.
